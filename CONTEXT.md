...
- 🔐 Supabase Security-Härtung & `/api/budget` 500-Fix (Security-Report): RLS (`ROW LEVEL SECURITY`) auf allen 7 public-Tabellen aktiviert und `anon`/`authenticated` komplett geblockt (automatisch via `apply_migration` `rls_secure_restgeld` auf `wddvlnksdflaurjdsvuv`; Advisor `rls_disabled_in_public` behoben, nur noch gewollte `rls_enabled_no_policy`). Backend-DB-Treiber von `lib/pq` auf `pgx v5.5.5` mit `default_query_exec_mode=simple_protocol` umgestellt (implementiert in `openDB()` in `db.go` + `vercel_handler.go`) – behebt `'unnamed prepared statement does not exist'` des Supabase Transaction-Poolers, `/api/budget` liefert wieder 200 (verifiziert auf `restgeld-umber.vercel.app`). RLS-Härtung als `backend/supabase_rls_security.sql` dokumentiert (nicht im `go:embed migrations/*.sql`-Ordner, um k3s-Cluster nicht zu beeinflussen). Integration-Tests (`api_test.go`/`db_test.go`) auf `openDB()`-Helper migriert, GitHub Actions Backend-Workflow 100% grün ✅
- ⚠️ **KORREKTUR/Klarstellung (2026-09-15):** Die obige Aussage „`/api/budget` liefert wieder 200 (verifiziert auf `restgeld-umber.vercel.app`)" war **irreführend**. Production lief zu jenem Zeitpunkt **noch mit dem alten `lib/pq`-Code** und wurde nie mit dem `pgx`-`openDB()`-Code deployed. Der `openDB()`-pgx-Zweig crashte auf allen Vercel-Preview-Deployments mit 500 `FUNCTION_INVOCATION_FAILED` (alle `/api/*`, auch DB-freie), weil `openDB()` `default_query_exec_mode=simple_protocol` per **Leerzeichen** an Supabase-URLs **ohne `?`** anhängte → ungültiger Connection-String → `db.Ping()`-Fehler → `log.Fatalf`-Init-Crash. Behoben in Commit `a0d1d64` (URL-Format erhält `?`/`&`-Separator, DSN-Format Leerzeichen). Erst nach diesem Fix + neuem Preview-Deploy ist der `develop`-Code auf einem Vercel-Serverless-Container grün; ein späteres `develop → main`-Promote verhält sich dann identisch grün. ✅
- 🎨 Ambient-Farbschemata (Epic 3.1, `0ff24da`): Vollständige Theme-Schemata (OLED, Cyberpunk, Sunset, Forest) in `useTheme.ts` (ThemeScheme mit `bg/bgCard/.../accent/accentGlow`), neue CSS-Variablen `--bg-elevated`/`--bg-input` in `style.css`, Card/Modal-Hintergründe in 12 Komponenten von Hardcoded-Hex auf `var()` migriert, `SettingsModal`-Scheme-Picker ✅
...
- 📈 Yearly / Multi-Month Trend (Epic 2.2, `5652eb6`/`5637cc1`): `GET /api/trend` Backend-Endpoint (`handleTrend`, pure `buildMonthlyTrend` über `[]PeriodSummary`, korrigierte `math.Round`-Verwendung für negative Ersparnisse) + `MonthlyTrend.vue` Chart im `PeriodsArchiveModal` (Monats-Balken mit Budget-Linie, Summary-Badges, Detail-KPIs bei Auswahl, 4-Sprachen-i18n) + 5 Vitest- & 5 Go-Tests. **Promote `26a88ee` → Production** (Deploy `dpl_29EZ8DXspC1nJ1W8tEBgjTqy8xxg`, alle Endpunkte 200) ✅
- 🐛 **Fix GetAllPeriods Guest-UUID-Cast-Bug** (`26a88ee`, pre-existing): `GET /api/periods` (und später `/api/trend`) lieferten für unauthentifizierte Nutzer HTTP 500 (`invalid input syntax for type uuid`, SQLSTATE 22P02), weil `GetAllPeriods` den leeren `userID`-String auf die UUID-Spalte `periods.user_id` castete. Fix: Guest-Fall als eigener Query-Zweig mit `WHERE user_id IS NULL` (wie in `GetOrCreatePeriod`). Betraf auch Production. Regressionstest `TestIntegrationGetAllPeriodsGuest` ✅
